...
const renderGrid = (across: Array<number>, down: Array<number>) => {
    ...
    for (let num of down) {
        ...
        for (let i = 0; i < size; i++)
            html += `<td class="wrong"><div class="sum hidden">${num} x ${across[i]}</div><div></div><input tabindex="${tabIndex++}" class="answer" max="${size * size}" min="1" id="${across[i]}:${num}" type="number"></td>`;
        html += '</tr>';
    }
